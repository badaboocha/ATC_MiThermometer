...
void write_memo(void) {
	...
	if (cfg.averaging_measurements == 1) {
		...
		mblk.humi = measured_data.humi_x1;
		...
		mblk.humi = (uint8_t)((summ_data.humi/summ_data.count + 50) / 100);
		mblk.relayStatus = (uint8_t)summ_data.relayStatus;
